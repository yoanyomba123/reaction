...
import { MetaData } from "./router/metadata"; // TODO fix circular dep
...
ReactionProduct.setProduct = (currentProductId, currentVariantId) => {
  ...
  if (product) {
    if (
      Router.getParam("handle") !== product.handle &&
      product.changedHandleWas &&
      product.changedHandleWas !== product.handle
    ) {
 ...
 * @return {Object|null} currently selected product cursor
 */
